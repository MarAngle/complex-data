...
type StatusItemInitOptionOption = {
  type?: 'default'
} | countOptionOption
...
class StatusItem extends Data {
  ...
  constructor (initOption: StatusItemInitOption) {
    initOption = formatInitOption(initOption, null, 'StatusItem未设置初始化数据')
  ...
  $initOption (option: StatusItemInitOptionOption = {}) {
    ...
    if (option.type == 'count') {
      if (option.prop) {
        ...
        this.$exportMsg(`StatusItem设置target类型需要传递taget目标值!`)
      }
    }
  }
  /**
