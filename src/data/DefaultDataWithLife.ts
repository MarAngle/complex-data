...
import { SimpleBuffer } from './SimpleData'

export interface DefaultDataWithLifeInitOption extends DefaultDataInitOption {
  prop?: string
  life?: LifeDataInitOption
}

export interface DefaultDataWithLifeBuffer extends SimpleBuffer {
  create: Record<string, undefined | boolean>
}

class DefaultDataWithLife<Buffer extends DefaultDataWithLifeBuffer = DefaultDataWithLifeBuffer> extends DefaultData<Buffer> implements DataWithLife {
  static $name = 'DefaultDataWithLife'
  static $formatConfig = { name: 'DefaultDataWithLife', level: 40, recommend: true }
  $prop: string
  $life!: Life
  constructor(initOption: DefaultDataWithLifeInitOption) {
    super(initOption)
    this._buffer.create = {}
    ...
    this._triggerCreateLife('DefaultDataWithLife', false, initOption)
    this._triggerCreateLife('DefaultDataWithLife', true, initOption)
  }
  /**
   * 触发创造生命周期
