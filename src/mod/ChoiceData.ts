...
class ChoiceData extends Data {
   ...
   * @returns {object}
   */
  formatItemFromList(id: idType, totalList: objectAny[], idProp = 'id') {
   ...
   */
  changeData(idList: idType[], currentList: objectAny[] = [], check: 'auto' | 'force' = 'auto', idProp: string) {
    // check 'auto'/'force'
   ...
   * @param {string} idProp id属性
   */
  addData(idList: idType[], list: objectAny[] = [], idProp: string, act: 'auto' | 'force' = 'auto') {
    if (act == 'auto') {
      for (let i = 0; i < idList.length; i++) {
        const id = idList[i]
