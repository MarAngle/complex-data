...
class PaginationData extends DefaultData {
  ...
  protected _autoCountTotal(unCountCurrent?: boolean, unTriggerCurrentLife?: boolean) {
    ...
    this._syncData(true, '_autoCountTotal')
  }
  /**
   * 获取总页码
   * @returns {number}
  ...
  _install(target: BaseData) {
    ...
    target.onLife('beforeReload', {
      ...
      data: (_lifeItem, _instantiater, force: ForceValue) => {
    ...
    this.onLife('change', {
      ...
      data: (_lifeItem, instantiater, prop, current) => {
